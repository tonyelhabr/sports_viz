...
library(tidyverse)
dir_proj <- 'c:/users/aelhabr/documents/projects/sports_viz/23-x'
path_misc_w_refs <- here::here(dir_proj, 'misc_w_refs.rds')
overwrite <- FALSE

if(!file.exists(path_misc_w_refs) & overwrite) {
  params <-
    list(
      country = 'ENG',
      gender = 'M',
  ...
  params
  urls <- purrr::invoke(worldfootballR::get_match_urls, params)
  results <- 
    purrr::invoke(worldfootballR::get_match_results, params) %>% 
    as_tibble() %>% 
    janitor::clean_names()
  results
  ...
  stats <-
    ...
    setNames(., .) %>% 
    map_dfr(
      ~worldfootballR:get_advanced_match_stats(.x, stat_type = 'misc', team_or_player = 'player'), .id = 'url'
    ) %>% 
    as_tibble() %>% 
    janitor::clean_names()
  stats
  ...
  # misc_w_refs %>% count(idx_stats, sort = TRUE) %>% count(n)
  write_rds(misc_w_refs, path_misc_w_refs)
} else {
  misc_w_refs <- path_misc_w_refs %>% read_rds()
}
