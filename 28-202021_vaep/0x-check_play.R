...
game_id <- 1485331
path_events <- file.path(dir_proj, sprintf('events_%s.rds', game_id))
path_meta <- file.path(dir_proj, sprintf('meta_%s.rds', game_id))
dir_data <- file.path('..', 'whoscraped', 'data', 'js')
paths <- 
  fs::dir_ls(dir_data, regexp = 'js') %>% 
  str_subset('2020-2021-West-Ham-Tottenham')
paths
n_path <- paths %>% length()
...
play_orig <-
  events %>% 
  filter(minute >= 9L & expanded_minute >= 9L) %>% 
  relocate(player_name) %>% 
  filter(event_id >= 90 & event_id <= 92L) %>% 
  select(player_name, id, event_id, minute, second, team_id, start_x = x, start_y = y, end_x, end_y, type_name, outcome_type_name)
play_orig
