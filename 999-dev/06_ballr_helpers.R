...
request <- httr::GET(players_url, httr::add_headers(request_headers))

players_data <- jsonlite::fromJSON(httr::content(request, as = "text"))
...
fetch_s_by_player_id_and_season <- function(player_id, season, season_type) {
  player_id <- 203081
  season <- "2019-20"
  ...
  raw_s_data <- data$resultSets[[1]]$rowSet
  col_names <- tolower(as.character(data$resultSets[[1]]$headers))

  if (length(raw_s_data) == 0) {
    s <- data.frame(
  ...
  return(list(player = s, league_averages = league_averages))
}
...
calculate_hex_coords <- function(s, binwidths) {
  xbnds <- hex_bounds(s$loc_x, binwidths[1])
  xbins <- diff(xbnds) / binwidths[1]
  ybnds <- hex_bounds(s$loc_y, binwidths[2])
  ...
  s_hb <- mutate(s, hexbin_id = hb@cID)
...
calculate_hexbins_from_s <- function(s, league_averages, binwidths, min_radius_factor, fg_diff_limits, fg_pct_limits, pps_limits) {
  
  
  binwidths = c(1.5, 1.5)
  min_radius_factor = .25
  ...
  grouped_s <- group_by(s, shot_zone_range, shot_zone_area)

  zone_stats <- grouped_s %>%
    summarize(
      zone_attempts = n(),
  ...
  hex_data <- calculate_hex_coords(s, binwidths = binwidths)
...
get_data <- function(id, seasons, season_type) {
  df <- fetch_s_by_player_id_and_season(id, seasons, season_type)

  s <- as.data.frame(df[1])
  ...
  hex_data <- calculate_hexbins_from_s(s, league_averages, binwidths = c(1.5, 1.5), min_radius_factor = .25, fg_diff_limits = c(-0.15, 0.15), fg_pct_limits = c(0.2, 0.7), pps_limits = c(0.5, 1.5))
