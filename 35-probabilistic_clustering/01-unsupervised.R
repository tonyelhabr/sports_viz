...
plot_metric_unsup <- function(.g) {
  ...
  viz <-
    mets_unsup_clean_filt %>% 
    ggplot() +
    aes(x = n, y = value, color = grp, alpha = k, group = sprintf('%s, %s', grp, k)) +
    geom_line(size = 1.2) +
    geom_point(size = 2, show.legend = FALSE) +
    ggrepel::geom_label_repel(
      data = mets_unsup_clean_filt %>% filter(n == max(n)),
      hjust = 'left',
      seed = 42,
      family = 'Karla',
      fontface = 'bold',
      ...
      label.size = NA,
      show.legend = FALSE,
      aes(label = sprintf('%s%s', k, ifelse(k == max(k), '  clusters', '')))
    ) +
    # scale_y_log10() +
    ...
    coord_cartesian(ylim = c(NA, NA), xlim = c(1, 17)) +
    theme_tony() +
    theme(
      legend.position = 'top',
    ...
    labs(
      ...
      subtitle = 'Unsupervised loss value as a function of components (x-axis) and clusters (line, transparency).',
      tag = lab_tag,
