...
do_clust <-
  function(.n,
           ...
           .f,
           .g,
           return = c('metrics', 'data', 'summary', 'clusters', 'assignments', 'fit'),
           overwrite = FALSE,
           ...) {
    # .f <- match.arg(.f)
    # .g <- match.arg(.g)
    ...
    if(return == 'data') {
      ...
      if(return == 'summary') {
        ...
        if(.g == 'kmeans') {
          ...
          probs_init <-
            class %>% 
            fastDummies::dummy_cols('.class', remove_selected_columns = TRUE)
          probs_init
...
pal <- c('PCA + kmeans'  = '#ef426f', 'PCA + GMM' = '#00b2a9', 'UMAP + kmeans' = '#ff8200', 'UMAP + GMM' = '#7a5195')
pal_clusts <-c('D' = '#003f5c', 'DM' = '#58508d', 'M' = '#bc5090', 'AM' = '#ff6361', 'F' = '#ffa600', 'G' = 'grey50')

do_uncertainty <- function(fit, .n, .k, ...) {
