...
preds <- file.path(dir_proj, 'preds-throw_ins-g_state+is_home.parquet') %>% 
  arrow::read_parquet() %>% 
  filter(period_id == 2L)
...
p <- preds %>% 
  ...
  labs(
    ...
    subtitle = glue::glue('The difference in time taken for throw-ins when <span style="color:{pal[["Ahead"]]}"><b>ahead</b></span> vs. <span style="color:{pal[["Behind"]]}"><b>behind</b></span> at the 90th minute.'),
...
path_viz <- file.path(dir_proj, 'throw_ins-g_state+is_home.png')
ggsave(
  plot = p,
  filename = path_viz,
  width = 12,
