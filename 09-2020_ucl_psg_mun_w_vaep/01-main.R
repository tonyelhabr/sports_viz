...
dir_proj <- '09-2020_ucl_psg_mun_w_vaep'
dir_data <- fs::path(dir_proj, 'data')
dir_data_raw <- fs::path(dir_proj, 'data-raw')
fs::dir_create(dir_data)
path_export_gif <- fs::path(printf('%s.gif', file))
...
path_export_vaep <- fs::path(dir_data, sprintf('%s_vaep.rds', file))
dir_anim <- fs::path(dir_data, 'anim')
...
get_config <- function(sheet) {
  path_config <- fs::path(dir_data_raw, 'config.xlsx')
  res <- 
    path_config %>% 
...
if(!fs::file_exists(path_export_vaep)) {
  events_nondribble <- 
    fs::path(dir_data_raw, 'CL2020_final8_events.csv') %>% 
    read_csv() %>% 
  ...
  expl <- score_model %>% DALEX::explain(data = as.matrix(vaep_features), y = as.numeric(vaep_labels$concedes))
  expl
  # expl <- fastshap::explain(score_model, X = as.matrix(vaep_features), y = as.numeric(vaep_labels$concedes))
  
  obs <- vaep_features[11, , drop = FALSE]
  res <- expl %>% DALEX::predict_parts_break_down(obs)
  expl <- lime::lime(vaep_features, score_model)
  expl %>% lime::explain(obs, ., n_features = 30) # %>% plot()
  events_vaep_init <-
    events_spadl_vaep %>% 
    filter(action_id >= 0)  %>% 
    mutate(
...
tracking <- 
  fs::path(dir_data_raw, 'CL2020_final8_tracking.csv') %>% 
  read_csv() %>% 
  select(-1) %>% 
  filter(play == play_filt)
tracking
