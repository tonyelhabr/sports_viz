...
# # g_state column gets coerced to a character later on.
...
states_by_team_slim <-
  states_by_team %>% 
  select(season, team, g_state, npxg_p90) # npxg_p90_diff) 
states_by_team_slim
...
states_by_team_clean <-
  ...
  mutate(
    lab = case_when(
      g_state == 'Neutral' ~ sprintf('%.2f', npxg_p90), #  npxg_p90_diff),
